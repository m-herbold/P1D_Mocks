...
def sample_ref_catalog(catalog, zmin, zmax, dz, n_per_bin, seed):
    with fitsio.FITS(catalog) as f:
        ...
        for i in range(nbins + 1):
            ...
            chosen = rng.choice(
                np.where(m)[0], size=int(N_per_bin), replace=False)
...
def initialize_qso_pool(sample, seed=123):
    ...
    for _, row in sample.iterrows():
        qso_pool[round(float(row["z_bin"]), 1)].append(
            row)  # round keys on ingestion
...
def build_redshift_template(catalog_path, zmin, zmax, dz,
    ...
    # robust against float drift
    bin_edges = np.arange(zmin, zmax + dz * 0.5, dz)
    ...
    total = counts.sum()
    weights = counts / total if (normalize and total >
                                 0) else counts.astype(float)
...
def plot_redshift_template(template: dict, outdir=None):
    ...
    # --- Left: raw counts ---
    axes[0].bar(bin_centers, counts, width=dz * 0.85,
    ...
    axes[1].bar(bin_centers, weights, width=dz * 0.85,
...
def apply_redshift_template(mock_path: str,
    ...
    if not rows:
        raise RuntimeError(
            "No rows selected — check redshift overlap between mock and template.")
    ...
    df["z_bin_index"] = np.searchsorted(
        bin_edges, df["z"].values, side="right") - 1
    df["z_bin"] = (template["zmin"] + template["dz"]
                   * df["z_bin_index"]).round(2)
...
def get_template_file(spec_path: str) -> str:
    ...
    matches = glob.glob(os.path.join(
        spec_path, "**", "transmission_*.fits"), recursive=True)
    if not matches:
        raise FileNotFoundError(
            f"No transmission FITS files found under: {spec_path}")
    return matches[0]
...
def save_downsampled_mock(
    ...
    with fitsio.FITS(mock_catalog_path, "r") as f_in, \
        ...
        selected_idx = np.array([mockid_to_idx[mid]
                                for mid in selected_mockids])
...
def simple_diagnostic_plot(output_file, outdir=None):
    ...
    for z in unique_z:
        ...
        z_bins.append(z)
        mean_trans.append(np.mean(trans_at_z) if len(
            trans_at_z) > 0 else np.nan)
    ...
    lines.append("-" * 46)
    lines.append(
        f"{'TOTAL':<10} {len(metadata):<10d} {valid_mean.mean():<12.6f}")
    stats_str = "\n".join(lines)
    ...
    for z, n in zip(z_bins, n_qsos):
        ax2.text(z, n + 0.1, str(n), ha='center',
...
def main():
    ...
    ap.add_argument("--spec_path", required=True,
                    help="Path to transmisison files")
    ap.add_argument("--z_template", required=False,
                    help="Apply redshift distribution downsampling to match catalog (if provided)")
    ap.add_argument("--catalog", required=False,
                    help="Catalog Reference for Metadata")
    ...
    ap.add_argument("--N", required=False,
                    help="Number of QSO per redshift bin to sample")
    ap.add_argument("--nside", required=False,
                    help="Nside (optional, default 16)")
    ap.add_argument("--pixel", required=False,
                    help="Pixel (optional, default 1)")
    ap.add_argument("--wmin", required=False,
                    help="Minimum output wavelength (default 3600)")
    ap.add_argument("--wmax", required=False,
                    help="Maximum output wavelength (default 9800)")
    ap.add_argument("--dwave", required=False,
                    help="Output wavelenth spacing (default 0.2, mimcks Ohio P1D mocks)")
    ap.add_argument("--diag_plot", required=False,
                    help="Optionally saves diagnostic plots")
    ...
    if args.N:
        ...
        all_trans_files = sorted(glob.glob(os.path.join(
            spec_path, "**", "transmission_*.fits"), recursive=True))
    ...
    if args.catalog:
        ...
        sample = sample_ref_catalog(
            ref_catalog, zmin, zmax, dz, n_per_bin, seed)
    else:
    ...
    for z_bin in redshift_bins:
        ...
        if z_key not in qso_pool or len(qso_pool[z_key]) < n_per_bin:
            print(
                f"  ⚠ z={z_bin}: Only {len(qso_pool.get(z_key, []))} QSOs available, skipping")
            continue
    ...
    if args.z_template and not args.catalog:
        raise ValueError(
            "--z_template requires --catalog to also be provided.")
    ...
    ## save file ##
    save_downsampled_mock(output_file, mock_sample,
                          nside, pixel, file_index, outdir)

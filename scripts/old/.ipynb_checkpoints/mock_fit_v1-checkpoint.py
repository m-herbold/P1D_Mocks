...
def main():
    ...
    parser.add_argument('--flux_file', type=str, help='Path to input (F,z) file (.txt or .fits) containing z and mean flux arrays')
    parser.add_argument('--z_target', type=str, required=True, help='Path to input file (.txt or .fits) containing target redshift values, OR a single float input')
    ...
    if not grouped_data:
        ...
        for z in z_target:
            if z in grouped_data:
                ...
                # print(f"Processing redshift {z}:")
                # print(f"k_values: {k_values}")
                # print(f"P1D_values: {P1D_values}")
    ...
    plt.plot(flux_z_array, flux_array, color='black', ls='--', label='Turner et al. 2024')
    plt.title('Mean Flux')
    plt.xlabel('z')
    ...
    for z in z_target:
        ...
        safe_z = str(zlist[redshift_index]).replace('.', '-')  # For naming figures
        ...
        plt.xlabel('k (s/km)')
        plt.ylabel(r'$P_{1D}$ ')
        plt.savefig(rf'{safe_z}_P1D_interpolation_test.png')
        ...
        result = least_squares(objective, xi_g_initial_guess, args=(zlist[redshift_index], xi_f_target))
        ...
        plt.figure()
        plt.semilogx(v_array_downsampled, xi_g_optimized, color='tab:orange', marker='o', label= r'$\xi_G$ fit @ z = '+str(zlist[redshift_index]))
        ...
        plt.figure()
        plt.plot(v_array_downsampled, xi_g_optimized, 'o', label='xi_g fit')
        ...
        plt.savefig(rf'{safe_z}_extended_xi_G_fit.png')

        ###################

        # recover xi_f
        xi_f_optimized_extrapolated = np.array([lognXiFfromXiG_pointwise(zlist[redshift_index], xi_g_i) for xi_g_i in xi_g_extrapolated])
        ...
        w_k = (k_arr > 1e-4) & (k_arr < 0.05)  # Window for k_arr
        w_fit_k = (mirrored_fit_k_arr > 1e-4) & (mirrored_fit_k_arr < 0.05)  # Window for fit_k_arr
        w_k_fine = (k_array_fine > 1e-4) & (k_array_fine < 0.05)  # Window for k_array_fine
        ...
        plt.loglog(k_arr[w_k], P1D_array[redshift_index, w_k], color='tab:blue', 
                   label=r'Input model, N = '+str(numvpoints))
        plt.loglog(k_array_fine[w_k_fine], p1d_fine[w_k_fine], 
                   label='interpolated / target, N='+str(new_size), color='tab:green', lw=3, alpha=0.7)
        ...
        plt.xlabel(r'k $[km/s]^{-1}$')
        plt.legend(fontsize=12, loc='lower left')
        plt.savefig(rf'{safe_z}_recovered_power_full.png')

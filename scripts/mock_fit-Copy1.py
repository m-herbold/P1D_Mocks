...
def PD13Lorentz_DESI_DR1(zlist):
    ...
    using per-redshift fitted parameters instead of a universal parameter set.
    """
    ...
    for i, z in enumerate(zlist):
        ...
        if not out_of_range[i] and np.abs(DESI_DR1_Z_BINS[iz] - z) > 0.05:
            print(f"  Warning: requested z={z:.3f} snapped to nearest DR1 bin z={DESI_DR1_Z_BINS[iz]:.3f}")
        A, n, alpha, B, beta, k1 = DESI_DR1_PARAMETERS[iz]
        lmd = 1.0 / k1**2
        p1d_dr1_fit[i] = evaluatePD13Lorentz((k_arr, z), A, n, alpha, B, beta, lmd)
...
def process_power_data(z_target, power_file=None, desi_dr=None):
    ...
    #     print("No (P,k,z) file provided, using default model (PD13Lorentz_DESI_EDR)")
        ...
        if desi_dr == 'DR1':
            print("No (P,k,z) file provided, using DESI DR1 model (per-redshift parameters)")
...
def save_xiG_half(v_array_half, xi_g_half, safe_z, save_cf=True):
    ...
    if save_cf:
        ...
        output_path = os.path.join(
            output_dir, f'{safe_z}_xiG_half_output-test.txt')
...
def mirror_xiG(safe_z, save_cf):
    ...
    if save_cf == 'full':
        full_cf_path = os.path.join(
            output_dir, f'{safe_z}_xiG_full_output-test.txt')
...
def save_PG(safe_z):
    ...
    # Load full CF data
    path_to_cf = os.path.join(cf_dir, f'{safe_z}_xiG_full_output-test.txt')
    ...
    # Save power spectrum
    export_filename = os.path.join(p1d_dir, f'P_G-{safe_z}-test.txt')
...
def plot_target_power(z, k_array_input, p1d_input, k_array_fine, p1d_fine):
    ...
    """
    print(rf'Saving: {z}_P1D_target-test.png')
    plt.figure()
    ...
    # save figure
    plt.savefig(rf'{z}_P1D_target-test.png')
...
def plot_target_xif(z, new_v_array, xif_interp_fit, v_array_downsampled,
    ...
    """
    print(rf'Saving: {z}_xi_F_target-test.png')
    plt.figure()
    ...
    # save figure
    plt.savefig(rf'{z}_xi_F_target-test.png')
...
def plot_xif_fit(z, v_array_downsampled, xi_f_target, xi_f_optimized, dv):
    ...
    """
    print(rf'Saving: {z}_xi_F_fit-test.png')
    plt.figure()
    ...
    plt.tight_layout()
    plt.savefig(rf'{z}_xi_F_fit-test.png')
    ...
    print(rf'Saving: {z}_xi_F_fit_residual-test.png')
    plt.figure()
    ...
    # save figure
    plt.savefig(rf'{z}_xi_F_fit_residual-test.png')
...
def plot_xig_fit(z, v_array_downsampled, xi_g_optimized,
    ...
    Saves:
        {z}_xi_G_fit.png: Plot comparing optimized, extrapolated, and
                          fixed-point xi_g.
    """
    print(rf'Saving: {z}_xi_G_fit-test.png')
    plt.figure()
    ...
    # save figure
    plt.savefig(rf'{z}_xi_G_fit-test.png')
...
def plot_xi_f_recovered(z, v_fine, xif_fine,
    ...
    """
    print(rf'Saving: {z}_xi_F_recovered-test.png')
    plt.figure()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(rf'{z}_xi_f_recovery-test.png')
...
def plot_recovered_power(z, k_array_input, p1d_input, w_k, mirrored_fit_k_arr,
    ...
    """
    print(rf'Saving: {z}_recovered_power-test.png')
    ...
    plt.savefig(rf'{z}_recovered_power-test.png')
    plt.close()
    ...
    for z in z_target:
        ...
        if args.plot_recovered_power:
            plot_recovered_power(safe_z, k_array_input, p1d_input, wk_display,
                                 ...
                                 redshift_index, delta_P_real, z, desi_dr=args.DESI_DR)
